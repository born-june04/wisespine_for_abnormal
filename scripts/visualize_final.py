...
if __name__ == '__main__':
    ...
    # Find all mask files from both datasets
    mask_files = []
    for dataset in ['dataset-verse19training', 'dataset-01training']:
        dataset_path = data_root / dataset / 'derivatives'
        if dataset_path.exists():
            mask_files.extend(sorted(dataset_path.glob('*/sub-*_seg-vert_msk.nii.gz')))
    
    print(f'Found {len(mask_files)} samples')
    
    # Process multiple samples (limit to 10 for memory)
    num_samples = min(10, len(mask_files))
    
    hardware_aug = SurgicalHardwareAugmenter(screw_probability=1.0, rod_probability=1.0, artifact_strength=0.8)
    fracture_aug = FractureAugmenter(compression_probability=1.0, compression_range=(0.3, 0.5))
    ...
    for idx in range(num_samples):
        ...
        ct_file = None
        for dataset in ['dataset-verse19training', 'dataset-01training']:
            candidate = data_root / dataset / 'rawdata' / subject_id / f'{subject_id}_ct.nii.gz'
            if candidate.exists():
                ct_file = candidate
                break
        
        if ct_file is None or not ct_file.exists():
            print(f'⚠️  Skipping {subject_id}: CT not found')
            continue
        
        print(f'\n[{idx+1}/{num_samples}] Processing {subject_id}...')
        ...
        try:
            ...
            print(f'  Hardware augmentation...')
            ct_hardware, mask_hardware = hardware_aug(ct_original.copy(), mask_original.copy(), spacing)
            visualize_hardware_detail(ct_original, ct_hardware, mask_original, mask_hardware, subject_id, 
                                      output_dir / f'hardware_{idx:02d}_{subject_id}.png')
            
            # Fracture augmentation
            print(f'  Fracture augmentation...')
            ct_fracture, mask_fracture = fracture_aug(ct_original.copy(), mask_original.copy(), spacing)
            visualize_fracture_detail(ct_original, ct_fracture, mask_original, mask_fracture, subject_id,
                                      output_dir / f'fracture_{idx:02d}_{subject_id}.png')
            
            # Clean up
            del ct_original, mask_original, ct_hardware, mask_hardware, ct_fracture, mask_fracture
            del ct_nifti, mask_nifti
            gc.collect()
    ...
    print(f'   - hardware_XX_*.png')
    print(f'   - fracture_XX_*.png')
